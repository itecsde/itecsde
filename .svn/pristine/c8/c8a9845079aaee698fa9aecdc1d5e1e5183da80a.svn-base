class Lre < ActiveRecord::Base
  ...
  # A lre may have many comments
  has_many :comments, :as => :commentable, :dependent => :destroy
  
  # A lre is categorised into many subjects
  has_many :lre_subject_annotations, :dependent => :destroy
  has_many :subjects, :through => :lre_subject_annotations
  accepts_nested_attributes_for :lre_subject_annotations, :allow_destroy => :true
  
  
  # A lre is bookmarkable
  ...
  # And a lre may include a representative picture
  attr_accessible :element_image, :name, :description, :url, :link, :lre_subject_annotations_attributes, :owner_type, :owner_id, :scraped_from, :creator_id, :tag_list
  ...
  def persist(idiomas_disponibles, prosa, manual_tags)
    ...
    if check_result == "insert"
      ...
      puts "UPDATE => SE ACTUALIZA EL LRE"
      scraped_lre = Lre.where(:url => self.url)[0]
      idiomas_disponibles.each do |idioma|
        I18n.locale = idioma
        scraped_lre.name = self.name
        scraped_lre.description = self.description
        scraped_lre.link = self.link
      end
      I18n.locale = "en"
      scraped_lre.extract_wikitopics(self.name, prosa, manual_tags)
      #scraped_lre.categorize(automatic_tags_ids)
      scraped_lre.info_to_wikify = prosa
      scraped_lre.hash_resource = recent_hash
      scraped_lre.save
      scraped_lre.reload
      Sunspot.index scraped_lre
      Sunspot.commit      
    elsif check_result == "do_nothing"
      puts "DO_NOTHING => NI SE INSERTA NI SE ACTUALIZA EL LRE, COMPROBAMOS TRADUCCIONES"
      if Lre.where(:url => self.url).size != 0
        lre = Lre.find_by_url(self.url)
        locales_string = Array.new
        lre.translated_locales.each do |locale|
        ...
        if (idiomas_sin_traducciones = idiomas_disponibles_string - locales_string) != []
          ...
          idiomas_sin_traducciones.each do |idioma|
            ...
            lre.name = self.name
            lre.description = self.description
            lre.link = self.link
            lre.save
          end
        else
