class BiographiesController < ApplicationController
  ...
  def index
    ...
    if (params[:advanced_search]=="1") 
      ...
      @search = Search.advanced_search_elements_by_type('Biography', search_text,search_text_en_translation, advanced_parameters,per_page,params[:page])
      ...
      @search = Search.listing_elements_by_type('Biography', search_languages, current_user, per_page,params[:page])
      @biographies = @search[:results]          
      @num_results = @search[:total]
...
=end      
  ...
  def update
    ...
    manual_tags = params[:biography][:tag_list].split(',')
    @biography.info_to_wikify = name + ". " + description
