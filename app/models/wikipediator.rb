...
class Wikipediator
 ...
 def esa_it(prose)
      begin
         #system 'java -cp /home/marcos/ESA/esa-lucene-min-prob.jar edu.wiki.demo.TestESAVectorsMinProbabilityCommandLine "' + prose.gsub(")","").gsub(/[^0-9a-z:,. ]/i, '') + '" > file_esa.txt'
         system 'java -cp /home/marcos/ESA/esa-lucene-min-prob.jar edu.wiki.demo.TestESAVectorsMinProbabilityCommandLine "' + prose.gsub(")","") + '" > file_esa.txt'
         line_num=0
         tags = Array.new
         File.open('file_esa.txt').each do |line|
            wikipedia_article_id = line.split("\t\t")[0].strip
            name = line.split("\t\t")[1].strip
            weight = line.split("\t\t")[2].strip
            puts line
            tags << {:name => name, :weight => weight, :wikipedia_article_id => wikipedia_article_id}
         end
         return tags
      rescue Exception => e
         puts "Exception in esa_it"
         puts e.message
         puts e.backtrace.inspect
      end
   end
  ...
  def metamap_it(prose)
     begin
        ...
        Dir.chdir("/media/storage/metamap/public_mm")
        #ejecutamos metamap
