class Report < ActiveRecord::Base
   ...
   # An report may have many comments
   has_many :comments, :as => :commentable, :dependent => :destroy

   # An report is categorised into many subjects
   has_many :report_subject_annotations, :dependent => :destroy
   has_many :subjects, :through => :report_subject_annotations
   accepts_nested_attributes_for :report_subject_annotations, :allow_destroy => :true

   # An report is bookmarkable
   ...
   # And an report may include a representative picture
   attr_accessible :element_image, :name, :description, :url, :link, :report_subject_annotations_attributes, :owner_type, :owner_id, :scraped_from, :creator_id, :tag_list
   ...
   def persist(idiomas_disponibles, prosa, manual_tags)
      ...
      #wikitopics = self.extract_wikitopics(self.name, prosa, manual_tags)
=begin
      classify = Classify.new
      section = classify.classify_it(wikitopics)
      if section != nil
...
=end      
      ...
      section = $classifier_naive_bayes_semantic.classify document_to_text_without_expanded_concepts(self,0.1)
