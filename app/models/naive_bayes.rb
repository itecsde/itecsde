...
class NaiveBayes
 ...
 # train the document
  def semantic_train(category, report, remove_outliers, threshold)
    classify = Classify.new
    if remove_outliers == true
       annotations = classify.remove_outliers(report, threshold)
    else
       annotations = report.taggable_tag_annotations
    end
    semantic_word_count(report, annotations).each do |word, count|
      @words[category][word] ||= 0
      @words[category][word] += count
      @total_words += count
      @categories_words[category] += count
  ...
  def semantic_probabilities(report, remove_outliers, threshold)
    probabilities = Hash.new
    classify = Classify.new
    if remove_outliers == true
       annotations = classify.remove_outliers(report, threshold)
    ...
    @words.each_key {|category|
      probabilities[category] = semantic_probability(category, report, annotations)
    }
    return probabilities
  end  
 ...
 # classfiy the document into one of the categories
  def classify(document, default='unknown')
   ...
   puts best[0].to_s + " " + second_best[0].to_s + " " + best[1].to_s + " " + second_best[1].to_s            
  ...
  def semantic_classify(report, remove_outliers, threshold, default = 'unknown')
    sorted = semantic_probabilities(report, remove_outliers, threshold).sort {|a,b| a[1]<=>b[1]}
    best,second_best = sorted.pop, sorted.pop
  ...
  def semantic_probability(category, report, annotations)
    semantic_doc_probability(category, report, annotations) * category_probability(category)
  end
  ...
  def semantic_doc_probability(category, report, annotations)
    doc_prob = 1
    semantic_word_count(report, annotations).each { |word| doc_prob *= word_probability(category, word[0]) }
    return doc_prob
  end
...
=begin    puts "***"
    puts "@words[category][" + word.stem + "]"
    puts @words[category][word.stem].to_f
    puts "@categories_words[" + category + "]"
=end    puts @categories_words[category].to_f

    #(@words[category][word.stem].to_f + 1)/@categories_words[category].to_f

    #(@words[category][word].to_f + 1)/@categories_words[category].to_f   
    
    (@words[category][word].to_f + 0.01)/@categories_words[category].to_f
    
    #@words[category][word.stem].to_f/@categories_words[category].to_f
  end
...
# get a hash of the number of times a word appears in any document
  ...
  def semantic_word_count(report, annotations)
    d = Hash.new
    #countries = Classify.generate_countries
    annotations.each do |annotation|
      #if !countries.include? annotation.tag.name.downcase
         key = annotation.tag.name
         d[key] = annotation.weight
      #end
    end
    #puts d
    return d
