 ...
 def scrape_material_type_oercommons(material_type_url, *reintentos)
   ...
   def mark_travel_well_lres(*reintentos)
      begin        
         ...
         subject_values = [91,104,144,195,209,219,292,303,383,9998,431,441,449,508,540,543,583,590,599,9999,707,716,790,798,856,875,968,974,978,1001,1017,1024,1040,1085,1139,1204,1227,1292]
         
         subject_values.each do |value|
            session.visit "http://lreforschools.eun.org/"
            sleep 0.5
            session.all('.iceSelOneMnu.portlet-form-field')[1].find("option[value='" + value.to_s + "']").select_option
            sleep 0.5
            ...
            while siguiente ==  true
               siguiente = false
               page_items = session.all(".icePnlGrp ul.search-list li .iceCmdLnk.resourceName").size        
               for i in 0..(page_items-1) do
                  session.all(".icePnlGrp ul.search-list li .iceCmdLnk.resourceName")[i].click
                  sleep 1
                  if i != 0
                     session.all(".icePnlGrp ul.search-list li .iceCmdLnk.resourceName")[i].click
                     sleep 1
                  ...
                  if Lre.where(:url => url).size > 0
                     lre = Lre.where(:url => url)[0]
                     lre.travel_well = true
                     lre.save
                  end       
                  session.evaluate_script('window.history.back()')
                  sleep 1
               end
               if session.all(".iceCmdLnk.next").size > 0
                  siguiente = true
                  session.all(".iceCmdLnk.next")[1].click
                  sleep 0.5
                  session.all(".iceCmdLnk.next")[1].click
                  sleep 0.5               
               else
