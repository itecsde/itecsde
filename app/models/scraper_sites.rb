    ...
    begin
      ...
      begin
        ...
        if reviews.to_i >= 5     
          ...
          reviews_text = ""
          reviews_page.css(".innerBubble").each do |review|
            if !review.css("div.entry p").empty?       
              reviews_text = reviews_text + " " + review.css("div.entry p")[0].text.strip
            end          
          end
          if reviews.to_i > 5 # => hay más de una pagina
            ...
            puts "SEGUNDA PAGINA DE REVIEWS"
            puts reviews_url
            reviews_page = Nokogiri::HTML(open(reviews_url))
            reviews_page.css(".innerBubble").each do |review|
              if !review.css("div.entry p").empty?       
