class Widget < ActiveRecord::Base
  ...
  def persist(idiomas_disponibles, prosa, manual_tags)
    ...
    if check_result == "insert"
      ...
      puts "UPDATE => SE ACTUALIZA EL WIDGET"
      scraped_widget = Widget.where(:url => self.url)[0]
      idiomas_disponibles.each do |idioma|
        I18n.locale = idioma
        scraped_widget.name = self.name
        scraped_widget.description = self.description
        scraped_widget.link = self.link
      end
      I18n.locale = "en"
      scraped_widget.extract_wikitopics(self.name, prosa, manual_tags)
      #scraped_widget.categorize(automatic_tags_ids)
      scraped_widget.info_to_wikify = prosa
      scraped_widget.hash_resource = recent_hash
      scraped_widget.save
      scraped_widget.reload
      Sunspot.index scraped_widget
      Sunspot.commit      
    elsif check_result == "do_nothing"
      puts "DO_NOTHING => NI SE INSERTA NI SE ACTUALIZA EL WIDGET, COMPROBAMOS TRADUCCIONES"
      if Widget.where(:url => self.url).size != 0
        widget = Widget.find_by_url(self.url)
        locales_string = Array.new
        widget.translated_locales.each do |locale|
