...
class ScraperLectures
  ...
  def scrape_ted_idioma(idioma,*siguiente)
    begin
      ...
      puts "Escrapeando Lectures en idioma " + idioma
      #I18n.locale = idioma
      puts url
      ...
      if siguiente[1]!=1
        page.css("div#list.talkListContainer dl.talkMedallion").each do |item|
          ...
          #sleep 2
          scrape_lecture_ted "http://www.ted.com" + item.css("dt.thumbnail a")[0]['href']
        end
      end
...
def scrape_lecture_ted(lecture_url, *reintentos)   
    begin
      name = ""
      description = ""
      puts lecture_url
      ...
      if !lecture_page.css("li.talk-more__item a#open-transcript.l3").empty?
        transcript_url = "http://www.ted.com" + lecture_page.css("li.talk-more__item a#open-transcript.l3")[0]['href']
        transcript_page =  Nokogiri::HTML(open(transcript_url))
        transcript_page.css("div.talk-article__header select.form-control option").each do |idioma|
        ...
        puts "Esta lecture esta disponible en estos idiomas: "
      ...
      idiomas_disponibles.each do |idioma|
        begin
          ...
          begin
            name = lecture_page.css("div.talk-hero__details h1.talk-hero__header div.talk-hero__title")[0].text.strip().gsub(/\n/," ").gsub(/\s+/,' ')
          ...
          begin
            description = strip_tags lecture_page.css("div.talk-top__details p.talk-description")[0].text.strip
          ...
          begin
            transcript = ""
            transcript_url = "http://www.ted.com" + lecture_page.css("li.talk-more__item a#open-transcript.l3")[0]['href'] + "?lang=" + idioma
            transcript_page =  Nokogiri::HTML(open(transcript_url))
            transcript_page.css("p.talk-transcript__para span.talk-transcript__para__text").each do |paragraph|
              transcript = transcript + paragraph.text.strip
            end
          rescue
            puts "Failed transcript"
          end
          
          info_to_wikify = name + ". " + description + ". " +  transcript + ". " #el wikipediador de momento razona sólo en ingles         
          info_to_wikify = info_to_wikify.gsub("\n"," ")
           
          scraped_lecture.info_to_wikify = info_to_wikify 
          scraped_lecture.link = lecture_url_locale              
        rescue Exception => e
          puts "Failed name-description-info_to_wikify idiomas ted"
          puts e.message
          puts e.backtrace.inspect
        end     
      end #do |idioma|
      ...
      begin    
        ...
        lecture_page.css(".talk-topics__list li.talk-topics__item a.l3").each do |tag|      
      ...
      begin
        author = lecture_page.css("div.talk-hero__speaker a.talk-hero__speaker__link")[0].text.strip
      rescue
        puts "Failed author ted"
      end  
        
      scraped_lecture.author = author   
      scraped_from = "http://www.ted.com"
      scraped_lecture.url = lecture_url
      scraped_lecture.scraped_from = scraped_from

      begin
        photo_url = lecture_page.css("div.talk-hero__poster img.talk-hero__image")[0]['src']
      rescue
        puts "Failed Photo ted"
      end
      ...
      prosa = scraped_lecture.info_to_wikify  
        
      scraped_lecture.persist(idiomas_disponibles, prosa, human_tags)
                
      begin
        speaker_bio_url = "http://www.ted.com" + lecture_page.css("div.talk-speaker__details div.talk-speaker__bio a.l3")[0]['href']
        scraper_biographies = ScraperBiographies.new
        #scraper_biographies.scrape_biography_ted speaker_bio_url
      rescue Exception => e
        puts "Failed Speaker Bio ted"
        puts e.message
        puts e.backtrace.inspect
      end              
      reintentos = 2  
      ...
      if reintentos > 0
        reintentos-=1
        sleep 3
        scrape_lecture_ted(lecture_url,1, reintentos)
      end
    end
 ...
 def scrape_lecture_videolectures(lecture_url, photo_url, *reintentos)
    ...
    begin
      ...
      begin    
        ...
        begin
          lecture_page.css("div#drilldown_list span#vl_desc div.lec_data a").each do |data|
            if data.previous_element.text.include? "author"
              author = data.text.strip
              break
            end
          end
        rescue
          puts "Failed author videolectures"
        end
