...
class ScraperPeople
  ...
  def scrape_scholar
    ...
    source = Source.create_or_select_source(source_name, "Person", "http://scholar.google.es")
    ...
    Subject.all.each do |expertise|
      ...
      scrape_category_scholar(url, 1, expertise.name)
      puts expertise.name
    end
  ...
  def scrape_category_scholar(category_url, input_number, area_of_expertise)
    puts input_number
    begin
        puts "check 1"
        if (input_number <= 2)
          ...
          category_page.css('div.g-unit').each do |person_block|
            ...
            scrape_person_scholar("http://scholar.google.es"+person_block.css('a.cit-dark-large-link')[0]['href'], area_of_expertise)
            puts "check 10"
          end
          puts "check 11"
          next_page = nil
          ...
          if (next_page != nil && next_page != "")
            puts "check 13"
            scrape_category_scholar("http://scholar.google.es"+category_page.css('div.cit-dgb').css('a.cit-dark-link')[1]['href'], number,area_of_expertise)
            puts "check 14"
          end
        end
    rescue
       puts "Fail scrape_category_scholar"
       scrape_category_scholar(category_url, input_number,area_of_expertise)
    end
  end
  
  def scrape_person_scholar(url, area_of_expertise)
    puts url
    name = ""
    description = ""
    ...
    puts "check scrape_person_scholar"  
        begin
          ...
          if check_result=="insert"
            ...
            scraped_person.area_of_expertise = area_of_expertise       
  ...
  def scrape_linkedin
    ...
    source_name = __method__.to_s    
    #Add new source, or select it if exists
    source = Source.create_or_select_source(source_name, "Person", "http://www.linkedin.com")    
