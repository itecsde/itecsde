...
function experience_form_init(){
	...
	popup_infinite_scroll_init('#book_assignment_selection #book_list','.extract_box', '"#book_assignment_selection .popup_body:first"', 'book_assignment_selection');	
...
function display_books(button, contribution_type) {
	$("#book_assignment_selection .element_list").html("");
	// Saves the current_position
	current_position = button;
	$.each($("#book_assignment_selection").find(".my_activities_filter"), function (index,value){
		$(value).removeClass("disabled");		
	}); 
	$($("#book_assignment_selection").find(".my_activities_filter")[0]).toggleClass("disabled");
	// Opens a popup to select an element
	open_area_popup("#book_assignment_selection",'/selectors/paginate_elements/?element_class=book_assignment_selection');
	$("#book_assignment_selection .books_list").masonry();
}
			...
			if ($(contribution_element).hasClass("deleted")){				
				...
				box_container.find(".component_container").each(function(component_index, component_element){
					...
					$(component_element).find(".text_container").each(function(text_index, text_container){
						...
						if ($(text_container).hasClass('deleted') && contribution_type != 'Application' && contribution_type != 'Event' && contribution_type != 'Biography' && contribution_type != 'Lecture' && contribution_type != 'Site' && contribution_type != 'Documentary' && contribution_type != 'Course' && contribution_type != 'Article' && contribution_type != 'Lre' && contribution_type != 'Post' && contribution_type != 'Slideshow' && contribution_type != 'Report'  && contribution_type != 'Artwork' && contribution_type != 'Book' && contribution_type != 'Widget') {
